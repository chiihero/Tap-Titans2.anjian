...
TracePrint iif(tribe_true, "部落:开启", "部落:关闭")	
//部落钻石选项
Dim tribe_true_num = ReadUIConfig("Checkbox_tribe_num",0)
TracePrint  "部落次数选择"&tribe_true_num
//仙女选项
Dim fairy_true = ReadUIConfig("Checkbox3")
TracePrint iif(fairy_true, "仙女:开启", "仙女:关闭 ")
//修改选项



Dim GG_skillstrue = ReadUIConfig("Checkbox4")
TracePrint GG_skillstrue
//TracePrint iif(GG_skillstrue, "修改技能:开启", "修改技能:关闭 ")
//修改选项
Dim GG_bluetrue = ReadUIConfig("Checkbox5")
TracePrint GG_bluetrue
TracePrint iif(GG_bluetrue, "修改蓝量:开启", "修改蓝量:关闭 ")
/*===============杂项===================*/
//等待开启时间
...
//邮箱
//发送邮箱账号
Dim m_username = ReadUIConfig("textedit_m_username",0)
//发送邮箱密码
Dim m_password = ReadUIConfig("textedit_m_password",0)
//接收邮箱账号
Dim m_tomail = ReadUIConfig("textedit_m_tomail",0)
//屏幕分辨率检测
...
/*****************************************************/
    ...
    temp5 = iif(GG_bluetrue or GG_skillstrue, "修改:开启", "修改:关闭 ")
    ShowMessage "分辨率: "&screenX&"*" &screenY &"\n层数:"&layer_number_max &"\n升级时间:" & update_time&"秒\n游戏重启时间:"&reboot_time_temp/60000 &"分钟\n"&temp1&"\n"&temp2&"\n"&temp3&"\n"&temp4&"\n"&temp5, 5000,screenX/2-275,screenY/2-550
    ...
    Call Competition()//比赛
    If tribe_true = True  AND tribe_true_num>0 Then
        Call tribe()
        Delay 1000
    End If 
    Call close_AD()//广告
    Call hum(4)//成就
    //减少高层数开始时的全面升级次数
    If ocrchar_layer > 9000 Then 
        update_main_num = 2
    ElseIf ocrchar_layer > 7500 Then 
        update_main_num = 1
    Else 
        update_main_num = 0
    End If
...
Function GG_init()
	//修改部分
    If GG_skillstrue = True or GG_bluetrue = True Then 
        Call GG()
        Dim error_num_one = 0
        While GG_flat = False
            Call kill_app()
...
Function kill_app()
    ...
    Do
        ...
        KeyPress "Back"
        Delay 2000
        FindColor 376, 1255, 408, 1286, "0C81FB", 0, 0.9, intX, intY
        If intX > -1 Then 
            Touch 758, 1264, 10
            Delay 3000
        End If
        ...
        If error_num_one > 10 Then 
            ...
            Exit do
        End If
    Loop While Sys.IsRunning("com.gamehivecorp.taptitans2") = True
    //等待修改器的确认游戏退出
    FindPic 91,736,992,1222, "Attachment:修改器确定.png","000000",0, 0.7, intX, intY
    error_num_one = 0
    Do
...
Function check_status()
    //出错重启
    If TickCount() - mistake_reboot > (20 * 60 * 1000) Then 
        TracePrint "出错重启"
        Call kill_app()
        GG_bluetrue = True
    ...
    If CmpColorEx("65|34|6D6859,990|1887|414424",1)= 1 Then  
        ...
        //检查蓝量
        Call ocrchar_blue(9)
        If CInt(Myblue(0)) < 50 Then 
            Call kill_app()
            GG_bluetrue = True
        End If
        Exit Function
    End If
    ShowMessage "界面被遮挡", screenX/2-280,screenY/4-200
    UseDict (0)
    ...
    If intX > -1 Then 
        Touch intX, intY, 10
    End If
    If Sys.AppIsFront("com.gamehivecorp.taptitans2") = False Then //Sys.isRunning("com.gamehivecorp.taptitans2") = False
        TracePrint "开启游戏"
        RunApp "com.gamehivecorp.taptitans2"
...
Function update_main(update_main_flat)
    ...
    If (update_time_main >= update_main_init_time) or update_main_flat <> 0 Then 
        ...
        Call close_AD()//广告   	
        FindColor 148,32,225,99,"A8B6E7-111111",0,1,intX,intY
        If intX > -1 and tribe_true = True Then
            Call tribe()
            Delay 1000
        End If
        //超过6000层之后达到最高层，不需要升级
        If ocrchar_layer < 8000 or update_main_num < 3 or update_main_flat=1 or updata_mistake >2 Then//update_main_num为超过6000层升级两次，update_main_flat为初始化升级，updata_mistake为防止卡层升级
            Call hum(3)//日常升级本人
            Call hum(1)//升级
            Call hero(1)//升级
...
Function layer_check()
    ...
    ElseIf Abs(ocrchar_layer - ocrchar_layer_temp) < 4 Then
        ...
        If TickCount() - auto_tribe_time > 300000 Then 
            ...
            Exit Function
            //自动升级
        ElseIf auto_updata_flat >= 2 or CmpColorEx("564|166|2663EF",0.9) = 1 Then
            TracePrint "自动升级"
...
Function hum(flat)
    ...
    //识别
    error_num_one = 0
    while CmpColorEx("92|1881|203C96",1) = 1
        TracePrint	"hum正在点开" 
        Touch 85, 1891, 100
        Delay 2000
        Call close_AD()//广告   	
        ...
        End If
    Wend
    If CmpColorEx("92|1881|203C96",1) = 0 Then 
        TracePrint	"hum已经点开"
        If flat = 1 Then 
            //升级
            Delay 200
...
Function hero(flat)
    ...
    Call close_AD()//广告
    error_num_one = 0
    while CmpColorEx("270|1899|615620",1) = 1	
        TracePrint	"hero正在点开"
        Touch 267,1890, 100
        Delay 2000
        Call close_AD()//广告
        ...
        End If
    Wend
    If CmpColorEx("270|1899|615620",1) = 0 Then 	
        If flat=1 Then 
            Call swipe_down(18)
            Delay 300
            Call update(2)
...
Function tribe()
    ...
    //        End If
    //    Wend
    FindColor 421,231,576,364,"00409A",0,1,intX,intY
    While intX = -1
        TracePrint"等待部落任务界面"
        Touch 204, 1749, 150
        Delay 2000
    ...
    Dim tribe_flat = 0
    FindColor 153,1755,357,1817,"30FFAC",0,0.9,intX,intY//识别旁边的时间
    Select Case tribe_true_num
    Case 0
        If intX = -1 and ocrchar_diamond = 0 Then 
            TracePrint"tribe_flat=True"
            tribe_flat=1
        End If
    Case 1
    ...
    Case 3
        ...
        End If
    End Select
    If timeX = -1 or tribe_flat>0  Then
        //点击“战斗”
        TracePrint"点击战斗"
        Touch 699,1767, 150
        Delay 1500
        If tribe_flat > 1 Then 
            Touch 723,1058, 150
            Delay 1000
            Touch 699,1767, 150
            Delay 1500
...
Function close_AD()
    ...
    If CmpColorEx("65|34|6D6859,990|1887|414424",1) = 0 Then 
        ...
        //    	Delay 1000
        Dim intX,intY
        FindColor 263,851,306,897,"FFFFD8|D7C575",0,1,intX,intY
        If intX > -1 Then 
            Call little_fairy()//小仙女
        Else 
...
Function little_fairy()
    ...
    ElseIf diamondX > -1 Then
        ...
        //判断收集字符出现
        error_num_one = 0
        FindColor 482,1448,646,1537,"CBA028-111111",0,1,intX,intY
        While intX = -1
            TracePrint "等待收集"
            Delay 1000
            //判断如果断网了的情况
...
Function little_fairy_rec
    //点击收集字符 
    Dim intX,intY,error_num_one
    FindColor 490,1448,590,1537,"CBA028-111111",0,1,intX,intY
    error_num_one = 0
    While intX > -1
        Tap 534, 1493
        TracePrint "收集"
...
Function close_window()
    ...
    Else 
        ...
        While CmpColorEx("519|252|191919", 1) = 1	
            ...
            End If 
        Wend
        FindColor 504, 469, 573, 499, "FFFFFF", 0, 1, closeX, closeY
        If closeX > -1 Then 
            TracePrint "捡取画面等待"
            Touch RndEx(closeX, closeX + 10), RndEx(closeY, closeY + 10), 50
            Delay 1000
            Touch RndEx(closeX, closeX + 10), RndEx(closeY, closeY + 10), 50
        End If
        If CmpColorEx("65|34|6D6859,990|1887|414424",1) = 0 Then 
...
Function skills
    ...
    error_num_one = 0
    While CmpColorEx("1009|1066|303845",1) = 1
        Touch 968, 1089, 50
        Delay 500
        error_num_one = error_num_one + 1
    ...
    Wend
    //技能6
    If CmpColorEx("975|1654|FFFFFF",0.9) = 1 And skill_6 = True Then
        Touch RndEx(946, 1027), RndEx(1682, 1755),RndEx(35, 55)
        Delay RndEx(20, 30)
    End If
    //技能5
    If CmpColorEx("795|1654|FFFFFF",0.9) = 1 And skill_5 = True Then
        Touch RndEx(772, 848), RndEx(1682, 1755),RndEx(35, 55)
        Delay RndEx(20, 30)
    End If
    //技能4
    If CmpColorEx("619|1654|FFFFFF",0.9) = 1 And skill_4 = True Then
        Touch RndEx(590, 666), RndEx(1682, 1755),RndEx(35, 55)
        Delay RndEx(20, 30)
    End If
    //技能3
    If CmpColorEx("440|1654|FFFFFF",0.9) = 1 And skill_3 = True Then
        Touch RndEx(406, 480), RndEx(1682, 1755),RndEx(35, 55)
        Delay RndEx(20, 30)
    End If	
    //技能2
    If CmpColorEx("260|1654|FFFFFF",0.9) = 1 And skill_2 = True Then
        Touch RndEx(264, 303), RndEx(1682, 1755),RndEx(35, 55)
        Delay RndEx(20, 30)
    End If
    //技能1
    Touch RndEx(80, 90), RndEx(1700, 1740), RndEx(20, 25)
    Delay RndEx(30, 50)
...
Function skills_use
    ...
    Dim checkX,checkY,error_num_one
    error_num_one = 0
    if CmpColorEx("1009|1066|303845",1) = 1
        Touch 968, 1089, 50
        Delay 500
    End If
    //技能6
...
Function prestige
    ...
    If pX > -1 And pY > -1 Then 
        ...
        Do
            TracePrint "点击第一层蜕变"
            FindPic 472,1443,564,1522,"Attachment:蜕变.png","000000",0,0.7,intX,intY
            //Touch 535,1504,100
            Touch intX, intY, 100
            Delay 1000
            error_num_one = error_num_one + 1
        ...
        Do
            TracePrint "点击第二层蜕变"
            FindPic 696,1244,743,1309,"Attachment:蜕变.png","000000",0,0.7,intX,intY
            Touch intX, intY, 100
            //Touch 757,1271, 100
            Delay 1000
            error_num_one = error_num_one + 1
    ...
    While ocrchar_layer >= old_ocrchar_layer
        ...
        ocrchar_layer=layer()
        error_num_one = error_num_one + 1
        If error_num_one > 10 And ocrchar_layer >= old_ocrchar_layer  Then 
            Call prestige()
            Exit Function
        elseif ocrchar_layer<layer_number_max*0.6  Then 
            TracePrint"蜕变成功跳出"
            Exit While
        ElseIf error_num_one > 30 Then
            TracePrint"出错"
            Exit While
        End If
    Wend
...
Function update(flat)
    Dim up1X,up1Y,checkX,checkY,boxX,boxY,last_check=-1,box_flat=0,case_1 = 0,case_3 = 0,error_num_one,error_num_two,error_num_three
    Dim intX,intY
    Call close_AD()//广告
    TracePrint "升级" &flat
    //购买框识别
    error_num_one=0
    FindColor 771,1152,823,1274, "525241", 1, 1, checkX, checkY//识别物品栏
    While (checkX = -1 And checkY = -1) or last_check = -1        
        //物品栏下箭头
        FindColor 997,1052,1025,1077,"303845",1,1,boxX, boxY
        ...
        If error_num_one > 40 Then 
            ...
            Exit While
        End If
        FindColor 771,1152,823,1274, "525241", 1, 1, checkX, checkY
    Wend
    Call close_AD()//广告
    Delay 150
End Function
Function update_one(flat)
    ...
    Select Case flat
    Case 1
        FindColor 926,1200,1054,1603, "0428A2-333333|003C96-333333|8A6400-333333", 5, 1, up1X, up1Y
    Case 2
        FindColor 926, 1200, 1054, 1791, "0428A2-333333|4A64BC-333333|003C96-333333", 5, 1, up1X, up1Y
    Case 3
        FindColor 926, 1200, 1054, 1791, "0428A2-333333|4A64BC-333333|003C96-333333", 5, 1, up1X, up1Y
    End Select
    error_num_one = 0
    While up1X > -1
        ...
        Touch up1X+5,up1Y+5, RndEx(10,15)
        Delay RndEx(35,55)
        Call close_AD()
        Select Case flat
        Case 1
...
Function ocrchar_blue(accuracy)
    ...
    Do
        ...
        If ocrchar <> "" Then  
            ...
            Else 
                blue_num = CStr(CInt(Myblue(0)) + 2) & "~" & CStr(CInt(Myblue(0)) + 30) & ";" & Myblue(1) & "::5"
            End If
            TracePrint blue_num
        Else 
            Call close_AD()//广告
...
Function change_database(intX, intY, str, flat)
    ...
    Delay 2000
    If flat = 2 Then 
        FindPic 158,453,248,877, "Attachment:修改器冻结.png","000000",0, 0.7, intX, intY
        If intX > -1 And intY > -1 Then 
            Touch intX, intY, 10
        End If
        Delay 1000	
    End If
    FindPic 854,867,943,1323, "Attachment:修改器是.png","000000",0, 0.7, intX, intY
    If intX > -1 And intY > -1 Then 
        Touch intX, intY, 10
    End If
End Function
...
Function search(flat)
    ...
    Delay 2000
    //新搜索
    FindPic 63,687,976,1382, "Attachment:修改器新搜索.png","000000",0, 0.7, intX, intY
    While intX > -1 And intY > -1 
    ...
    Delay 2000
    //隐藏
    FindPic 816,542,928,1243, "Attachment:修改器隐藏.png","000000",0, 0.7, intX, intY
    error_num_one = 0
    ...
    While intX > -1
        ...
        If error_num_one > 300 Then 
            ...
            Exit While
        End If
        FindPic 763,1112,979,1250, "Attachment:修改器隐藏.png","000000",0, 0.7, intX, intY
    Wend
    TracePrint "等待搜索结束"
    Delay 1500
    FindPic 643,528,759,1274, "Attachment:修改器取消.png", "000000", 0, 0.7, intX, intY
    If intX > -1 Then 
        TracePrint "点击取消"
        Touch intX, intY, 10
    End If
...
Function chest
    TracePrint "区域宝箱"
    Dim intX,intY
    FindPic 2,401,128,907,"Attachment:宝箱.png","000000",1,0.7,intX,intY
    If intX > -1 And intY > -1 Then 
        TracePrint "发现宝箱"
        TracePrint intX
        TracePrint intY
...
Function Competition
    TracePrint "比赛"
    Dim intX,intY,error_num_one = 0
    FindColor 46,192,98,222,"201100|000B38",0,1,intX,intY
    If intX > -1 And intY > -1 Then
        Touch RndEx(intX, intX + 10), RndEx(intY, intY + 10), 100
        error_num_one = 0
        While CmpColorEx("533|1748|C29926", 0.9) = 0
            Delay 1000
            error_num_one = error_num_one + 1
            If error_num_one > 10 Then 
                TracePrint"出错"
...
Function mail(subject)
    TracePrint "邮箱"
    If m_username = 0 or m_password = 0 or m_tomail = 0 Then 
        TracePrint "邮箱信息不全"
        Exit Function
    End If
    Dim error_num_one
...
End Function
//封装while函数
Function whilee(judge, max_error)
    //	Dim error_num_one = 0
    //	While judge
    //		error_num_one = error_num_one + 1
    //	    If error_num_one > max_error Then 
    //            TracePrint"出错"
    //            Exit While
    //        End If
    //    Wend
