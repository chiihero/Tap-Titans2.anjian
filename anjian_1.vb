...
TracePrint iif(GameGuardian_true, "修改:开启", "修改:关闭 ")
/*===============杂项===================*/
//等待开启时间
...
Function tribe()
	...
	Case 0
    	If ocrchar_diamond = 0 and timeX = -1 Then 
    		tribe_flat=True
    	End If
	Case 1
...
Function little_fairy()
    ...
    Elseif diamondX > -1 Then//不看
        ...
        //判断收集字符出现
        error_num_one = 0
		FindPic 467,1447,601,1541,"Attachment:小仙女收集.png","000000",0,0.9,intX,intY
        While intX = -1
            TracePrint "等待收集"
            Delay 1000
            //判断如果断网了的情况
...
//		Loop While intX > -1
    ...
    While ocrchar_layer >= old_ocrchar_layer
        ...
        ocrchar_layer=layer()
        error_num_one = error_num_one + 1
        If error_num_one > 20 And ocrchar_layer = old_ocrchar_layer  Then 
            Call prestige()
            Exit Function
        elseif ocrchar_layer<layer_number_max*0.6  Then 
            TracePrint"蜕变成功跳出"
            Exit While
        ElseIf error_num_one > 50 Then
        	TracePrint"出错"
            Exit While
        End If
    Wend
...
Function change_database(intX, intY, str, flat)
	...
	Delay 2000
	If flat = 2 Then 
		FindPic 158,453,248,877, "Attachment:修改器冻结.png","000000",0, 0.8, intX, intY
		If intX > -1 And intY > -1 Then 
			Touch intX, intY, 10
		End If
		Delay 1000	
	End If
	FindPic 854,867,943,1323, "Attachment:修改器是.png","000000",0, 0.8, intX, intY
	If intX > -1 And intY > -1 Then 
		Touch intX, intY, 10
	End If
End Function
...
Function search(flat)
	...
	Delay 2000
	//新搜索
	FindPic 63,687,976,1382, "Attachment:修改器新搜索.png","000000",0, 0.8, intX, intY
	If intX > -1 And intY > -1 Then 
	...
	Delay 2000
	//隐藏
	FindPic 816,542,928,1243, "Attachment:修改器隐藏.png","000000",0, 0.8, intX, intY
	error_num_one = 0
	...
	While intX > -1
        ...
        If error_num_one > 300 Then 
            ...
            Exit While
        End If
		FindPic 763,1112,979,1250, "Attachment:修改器隐藏.png","000000",0, 0.8, intX, intY
	Wend
	TracePrint "等待搜索结束"
	Delay 1500
	FindPic 643,528,759,1274, "Attachment:修改器取消.png", "000000", 0, 0.8, intX, intY
	If intX > -1 Then 
		TracePrint "点击取消"
		Touch intX, intY, 10
	End If
